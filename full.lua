...
FDtracking = CreateFrame('Frame')
FDtracking:RegisterEvent('PLAYER_AURAS_CHANGED')
local hasFD = false
local FDframe = nil
FDtracking:SetScript('OnEvent', function()
	
	local FD = false
	for i=1,40 do 
		local n,_,tex=UnitBuff("player",i);
		if tex == "Interface\\Icons\\Ability_Rogue_FeignDeath" or n == "Feign Death" then 
			FD = true
		end 
	end
	
	if hasFD == true and FD == false and FDframe ~= nil then
		--print('ding')
		OmniCC:StartTimer(FDframe, GetTime(), 30)
	end
...
do
	...
	function OmniCC:HookCooldown()
		...
		hooksecurefunc(methods, 'SetCooldown', function(self, start, duration)
			...
			local s,d,a = GetSpellCooldown("Feign Death")
			if FDframe == nil and a == 1 and Parent:IsVisible() then
				--print(s..'-'..d..'-'..a)
				if duration == d and start == s then
					FDframe = self
					--print(self:GetName())
				end
			end
